    ...
    for (; w.element(g);) {
      for (let x = 0; x < b.length; x++) {
        ...
        if (Jt(g, I) && Kt(N, y) && Kt(N, g)) {
          ...
          v.currentTarget = g;
          for (const P of D) Ke(P.options, p) && P.func(v)
        }
      }
      g = zt(g)
    }
...
function Qa(s) {
  ...
  return t.getPointerAverage = Fo, t.getTouchBBox = $a, t.getTouchDistance = Pa, t.getTouchAngle = La, t.getElementRect = zs, t.getElementClientRect = $o, t.matchesSelector = Jt, t.closest = Io, t.globalEvents = {}, t.version = "1.10.27", t.scope = s, t.use = function(e, i) {
...
const fc = {
  ...
  install(s) {
    ...
    s.usePlugin(Zo), s.usePlugin(Ph), t.modifiers = Vi;
    for (const e in Vi) {
      const {
        _defaults: i,
        _methods: n
  ...
  Rc = (s, t) => {
    ...
    const r = tt(!1),
      ...
      k = () => {
        var J;
        const T = N(),
          W = Math.ceil(((((J = window.visualViewport) == null ? void 0 : J.width) ?? window.innerWidth) - T) / 2);
        return {
          left: W,
          right: W
        }
      ...
      I = () => {
        const T = document.querySelector(".tabs"),
          ...
          J = (T == null ? void 0 : T.offsetHeight) ?? 0;
        return {
      ...
      D = () => {
        ...
        return Math.ceil((((J = window.visualViewport) == null ? void 0 : J.height) ?? window.innerHeight) - T - W)
      },
      P = () => {
        const T = N(),
          W = D();
      ...
      C = () => {
        ...
        const T = N(),
          ...
          J = T - W < 0 ? 0 : Math.ceil(T - W),
          wt = 0;
        p.position.x = J, p.position.y = wt, s.value.style.transform = `translate(${J}px, ${wt}px)`
      },
      L = () => {
      ...
      H = async () => {
        ...
        requestAnimationFrame(async () => {
          V() ? (at(), await et(), F(`${P()}px`, `${z()}px`)) : L(), await d(n)
        })
      }, U = () => {
        ...
        onmove(T) {
          var W, J;
          p.position.x += ((W = T.deltaRect) == null ? void 0 : W.left) ?? 0, p.position.y += ((J = T.deltaRect) == null ? void 0 : J.top) ?? 0, T.target.style.height = `${T.rect.height}px`, T.target.style.transform = `translate(${p.position.x}px, ${p.position.y}px)`, T.target.style.width = `${T.rect.width}px`
        },
        onstart: u,
        onend(T) {
          const W = P(),
            J = z();
          T.rect.width > W || T.rect.height > J ? (T.target.style.height = `${J}px`, T.target.style.width = `${W}px`, d(n).then(() => c(T))) : c(T)
        }
      }, et = async () => {
        var T, W, J;
        b(), v(), L(), C(), n = (W = (T = a(s.value)) == null ? void 0 : T.draggable(ct)) == null ? void 0 : W.resizable(K), o = (J = a(t.value)) == null ? void 0 : J.on("doubletap", async wt => {
          V() ? L() : F(`${P()}px`, `${z()}px`), await d(n), wt.preventDefault()
    ...
    setup(s) {
      ...
      return (r, l) => {
        ...
        return _(), E("div", Hc, [S("button", {
          ...
          default: Y(() => [S("div", Xc, [S("div", Jc, A(("t" in r ? r.t : m(O))("memepad.jetton.steam.start_sheet.title")), 1), S("div", Yc, A(("t" in r ? r.t : m(O))("memepad.jetton.steam.start_sheet.subtitle")), 1), S("div", Gc, [m(e).status === "loaded" ? (_(), E("button", {
            key: 0,
            type: "button",
            class: "reset",
...
class wn {
  ...
  Gt(t, e, i, n) {
    ...
    const z = d > 0 ? Math.max(1, Math.floor(d * h)) : 0,
      ...
      H = Math.floor(F + D / 2 - P / 2),
      nt = H + P,
      U = n === "right",
      Z = U ? a.width - d : d,
      X = U ? l.width - z : z;
    let ot, ct, K;
    return U ? (ot = X - V, ct = X - C, K = Z - u - v - d) : (ot = X + V, ct = X + C, K = Z + u + v), {
      Qt: U,
      Jt: {
        ii: H,
        li: F,
      ...
      if (o) {
        if (this.zt.fe) {
          ...
          let D = Math.max(v, Math.round(l.ve * i) - a),
            P = D + u - 1;
          P > v + y - 1 && (P = v + y - 1, D = P - u + 1), t.fillRect(N, D, c - N, P - D + 1)
        }
        const x = h + g;
...
const ho = "chart-series-type-storage-key",
    ...
    const t = It("$iBkM1fJygO"),
      ...
      d = g => {
        ...
        const N = g.points.filter(P => I.data.points.some(z => z.timestamp === P.timestamp)),
          D = g.points.filter(P => !I.data.points.some(z => z.timestamp === P.timestamp));
        return {
          type: x,
          interval: k,
          data: {
            points: [...D, ...I.data.points.map(P => N.find(z => z.timestamp === P.timestamp) || P)]
          }
        }
      },
      f = async g => {
  ...
  _p = st({
    ...
    setup(s) {
      ...
      const D = (z, V, C) => {
          var nt;
          const L = (nt = $e().$webApp.dataUnsafe.user) == null ? void 0 : nt.language_code;
          u.value || (u.value = hp(V, {
            localization: {
              timeFormatter: U => {
                const Z = ot => String(ot).padStart(2, "0"),
                  X = new Date(U * 1e3);
                return `${Z(X.getUTCHours())}:${Z(X.getUTCMinutes())}`
              },
              priceFormatter: U => {
                if (g.value === Lt.MARKET_CAP) return `$${Ut(U,{accuracy:0})}`;
                {
                  if (Math.abs(U) < 1e-9) return "$0";
                  const [Z, X] = U.toFixed(9).split("."), ot = X[0], K = X.slice(1).match(/^0+/), et = K ? K[0].length : 0, at = et > 0 ? String.fromCharCode(8321 + et) : "", T = X.slice(et + 1).replace(/0+$/, "").slice(0, 3);
                  return `$${Z}.${ot}${at}${T}`
                }
              },
              locale: L
          ...
          (!d.value || N !== C) && (d.value && u.value.removeSeries(d.value), C === oe.CANDLESTICK ? d.value = u.value.addCandlestickSeries({
            ...
            const Z = (K = (ct = d.value) == null ? void 0 : ct.dataByIndex(0)) == null ? void 0 : K.time,
              X = (at = (et = u.value) == null ? void 0 : et.timeScale().getVisibleRange()) == null ? void 0 : at.from;
            if (!Z || Z !== X) return;
            const ot = (() => {
        ...
        P = j(() => {
          ...
          const z = Fr(t.jettonPnl.pnlAbsoluteUSD, {
              accuracy: 2,
              trim: !0
            }),
      ...
      return he(() => {
        ...
        return _(), E("div", cp, [S("div", up, [S("div", {
          ...
          class: "dot"
        }, null, -1)), S("div", bp, A(("t" in z ? z.t : m(O))("memepad.jetton.chart.coming_text")), 1)])])) : (_(), E("div", yp, [m(P) ? (_(), E("div", {
          key: 0,
  ...
  jp = st({
    ...
    setup(s) {
      ...
      return Hr(go, {
        ...
        }, [Ft((_(), E("div", Lp, [Yr(n.$slots, "default", {}, void 0, !0), (_(!0), E(dt, null, de(m(i), a => (_(), B(r, {
          key: a.value,
          label: a.label,
          value: a.value,
          type: s.type
  ...
  Gp = st({
    ...
    setup(s, {
      ...
      const P = Qs(C => Bt().memepad.memepadJettonTradeInputUpdate(C), 300).debouncedFunction;
      he(h, C => P({
        value: C.toNumber(),
        type: n.value
      })), We(() => {
      ...
      function V() {
        ...
        return async () => {
          ...
          await (async () => {
            ...
            if (K.err) {
              Zr().error("Memepad jetton trade error", K.extra), o.value[F] = !1, ct(), L({
                value: H.toFixed(),
                operation: F
              }, {
      ...
      return (C, L) => {
        ...
        return _(), B(et, {
          ...
          inner: Y(() => [S("div", Hp, [(_(), B(nt, {
            ...
            centered: "",
            onBlur: g
          }, null, 8, ["modelValue", "postfix"])), S("div", Xp, [m(k) ? (_(), E("span", Jp, A(m(k)), 1)) : m(k) === void 0 ? (_(), B(U, {
            key: 1,
            class: "skeleton"
          })) : ht("", !0)]), S("button", {
            type: "button",
          ...
          "bottom-action": Y(() => [R(ct, {
            ...
            default: Y(() => [C.unlockEnabled ? (_(), B(X, {
              key: 0,
              jetton: C.jetton,
  ...
  Am = st({
    ...
    setup(s) {
      const t = el(),
        ...
        o = j(() => il.Address.parse(e.value.address).equals(e.value.masterContractAddress)),
        r = j(() => {
          var D;
          return (D = sl().getters.getUser.value) == null ? void 0 : D.referrerToken
        }),
        {
          addresses: l,
      ...
      function N() {
        ...
        function H() {
          const X = () => !x.value || h.value.status !== "connected" ? ie() : Mt({
            jettonSdk: x.value
          });
          os(X, () => {
            V = Ri(async () => {
          ...
          os(ot, () => {
            C = Ri(async () => {
              ...
              return Mt(gt(et) ? et.data : P.value)
            }, K => P.value = K, F.UNLOCK_ENABLED_UPDATE), C == null || C.exec()
          });
        ...
        return {
          init: H,
          destroy: () => {
            V == null || V.destroy(), C == null || C.destroy(), L == null || L.destroy(), D.value = void 0, P.value = void 0, z.value = void 0
          },
          update: () => {
            V == null || V.exec(), C == null || C.exec(), L == null || L.exec()
          },
          resetAndUpdateUnlocked: () => {
            P.value = void 0, C == null || C.exec()
          },
      ...
      return (D, P) => {
        ...
        return !m(n) || !m(l) || !m(x) || m(n).status === m(tn).IN_PROGRESS ? (_(), B(z, {
          ...
          onTradeSuccess: b,
          onUnlockSuccess: y,
          onShare: P[0] || (P[0] = L => D.$emit("share", "trade")),
          onCopy: P[1] || (P[1] = L => D.$emit("copy", "trade"))
