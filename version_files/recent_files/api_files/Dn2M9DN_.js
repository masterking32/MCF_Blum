...
class z extends P {
  ...
  async getJettonOverview(t) {
    ...
    const {
      ...
      fees: _.toNano(0),
      coins: _.toNano(0)
    } : await this.getFirstCoinsForTons(t), f = this.getCoinPriceInTons(await this.getFirstCoinsForTons(1)), S = f.mul(E), C = new h(o(F.coins)), N = new h(o(F.fees)), U = new h(a).minus(N), I = U.div(n).times(100), B = C.div(E).times(100), x = new h(E).sub(C), O = {
      share: a,
      tonForDexWithFees: r,
      tonForDex: n,
      tonCollected: U,
  ...
  async createJetton(t) {
    ...
    const a = new h(t.share),
      ...
      E = {};
    e.socials.length && (E.social_links = JSON.stringify(e.socials.map(C => C.url))), e.bannerFileKey && (E.cover_image = M(e.bannerFileKey));
    const T = {
        authorAddress: r,
        name: n.name,
      ...
      S = await m(async () => await this.sdk.deployCoin(f, T, F))();
    return g(S) ? l({
      queryId: s.data.queryId,
...
class Q extends P {
  ...
  async getBuyInfo(t) {
    const e = await m(async () => await this.coin.getCoinsForTons(w(t)))();
    if (!g(e)) return p();
  ...
  async getSellInfo(t) {
    const e = await m(async () => await this.coin.getTonsForCoins(w(t)))();
    if (!g(e)) return p();
  ...
  async getTradeTransactionStatus(t) {
    ...
    const s = e.data.actions.find(a => {
      var i;
      return a.type === "SmartContractExec" && ((i = a.SmartContractExec) == null ? void 0 : i.contract.address.equals(_.Address.parseFriendly(t.jettonAddress).address))
    });
...
const X = () => ({
  ...
  L = () => {
    const d = A("$JwHI1YwVmt"),
      c = A(() => D(), "$VQxqx8b060"),
      t = A(() => G(async () => W(await R.getMemepadPartnerAddresses()), a => {
    ...
    return {
      ...
      partnerAddressesResolver: v(() => c.value)
    }
  },
  te = () => {
    const d = A("$6Ey6bBtimr"),
      ...
      s = k(async () => {
        ...
        return c.value = t(n), W(await c.value.getBclData())
      }, n => d.value = n);
    return {
      init: async () => {
        await s.exec()
      ...
      bclData: v(() => d.value),
      memepadManagementSdk: v(() => c.value)
    }
  },
  re = () => {
    const {
