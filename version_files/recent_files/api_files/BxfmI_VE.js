...
class $ extends P {
  ...
  async createJetton(t) {
    const e = this.getMemepadJettonInfoFromForm(t),
      s = await D.saveMemepadJettonInfo(e);
    if (!m(s)) return E();
...
const V = () => ({
    ...
    getDexTradingSdk: e => new j(e)
  }),
  W = () => {
    const c = _("$JU3mIPe8CH"),
      i = _(() => J(), "$P1Vozdfyic"),
      t = _(() => q(async () => b(await D.getMemepadPartnerAddresses()), a => {
  ...
  z = () => {
    const c = _("$JwHI1YwVmt"),
      ...
      s = k(async () => {
        ...
        return i.value = t(n), b(await i.value.getBclData())
      }, n => c.value = n);
    return {
      init: async () => {
        await s.exec()
