...
class z extends W {
  ...
  async getJettonOverview(t) {
    ...
    const {
      ...
      fees: _.toNano(0),
      coins: _.toNano(0)
    } : await this.getFirstCoinsForTons(t), A = this.getCoinPriceInTons(await this.getFirstCoinsForTons(1)), S = A.mul(E), C = new h(o(F.coins)), N = new h(o(F.fees)), U = new h(a).minus(N), I = U.div(n).times(100), B = C.div(E).times(100), x = new h(E).sub(C), b = {
      share: a,
      tonForDexWithFees: r,
      tonForDex: n,
      tonCollected: U,
  ...
  async createJetton(t) {
    ...
    const T = {
      ...
      S = await m(async () => await this.sdk.deployCoin(A, T, F))();
    return g(S) ? l({
      queryId: s.data.queryId,
...
const X = () => ({
  ...
  ee = () => {
    ...
    const e = q(),
      ...
      }, a = w(() => H(s, r => d.value = r, 6e4 * 2), "$5jtlbZZRyA");
    return {
      init: async () => {
        t = $(() => {
          var r;
  ...
  L = () => {
    const d = w("$JwHI1YwVmt"),
      c = w(() => D(), "$VQxqx8b060"),
      t = w(() => G(async () => P(await R.getMemepadPartnerAddresses()), a => {
