  ...
  ue = O({
    ...
    setup(ce) {
      ...
      const c = p(() => o.state.value.status === "loading" ? {
        ...
        S = async () => {
          const e = Z("from");
          e && re(), m.name === "memepad-jetton-shortname" && v.replace({
      ...
      return le(() => m.name === "memepad-jetton-shortname-transactions" ? h().push({
        name: "memepad-jetton-shortname-trade"
        ...
        name: "memepad"
      })), se(p(() => o.state.value.status !== "loaded" || !!n.controls.value.value), "memepad-jetton-modal-state"), V().useTabsOnPage(p(() => {
        var t;
