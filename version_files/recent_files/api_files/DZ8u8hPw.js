...
class $ extends W {
  ...
  async createJetton(t) {
    const e = this.getMemepadJettonInfoFromForm(t),
      s = await M.saveMemepadJettonInfo(e);
    if (!m(s)) return E();
    const a = new p(t.share),
      ...
      g = {};
    e.socials.length && (g.social_links = JSON.stringify(e.socials.map(S => S.url))), e.bannerFileKey && (g.cover_image = U(e.bannerFileKey));
    const A = {
        authorAddress: d,
        name: n.name,
...
const V = () => ({
  ...
  P = () => {
    const c = _("$JU3mIPe8CH"),
      i = _(() => J(), "$P1Vozdfyic"),
      t = _(() => q(async () => D(await M.getMemepadPartnerAddresses()), a => {
