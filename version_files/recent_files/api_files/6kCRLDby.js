...
class Ji {
  ...
  Gt(t, e, i, s) {
    ...
    } = t, u = this.zt.hi || !this.zt.mi ? e.T : 0, d = this.zt.bi ? e.C : 0, f = e.L + this.Xt.wi, p = e.B + this.Xt.gi, v = e.A, g = e.I, y = this.zt.Kt, S = e.P, V = i.Mi(r, y), x = Math.ceil(i.xi(r, y)), N = S + f + p, M = e.C + v + g + x + u, z = Math.max(1, Math.floor(c));
  ...
  X(t, e) {
    ...
    t.useBitmapCoordinateSpace(({
      ...
      const g = Math.round(a * f),
        ...
        V = Math.round(u * p),
        x = Math.round(2 * f);
      if (d.beginPath(), d.moveTo(g, y), d.lineTo(g, V - x), d.arcTo(g, V, g + x, V, x), d.lineTo(S - x, V), d.arcTo(S, V, S, V - x, x), d.lineTo(S, y), d.fill(), v.hi) {
        const N = Math.round(v.ki * f),
          M = y,
          z = Math.round((M + e.T) * p);
        d.fillStyle = v.V;
        const E = Math.max(1, Math.floor(f)),
          W = Math.floor(.5 * f);
        d.fillRect(N - W, M, E, z - M)
      }
    }), t.useMediaCoordinateSpace(({
      context: d
    }) => {
...
class hn extends gt {
  ...
  K(t) {
    ...
    o !== void 0 && nn(t, e, o, i, s, c, Rr), l && function(u, d, f, p, v) {
      ...
      let V = null;
      const x = Math.max(1, Math.floor(g)) % 2 / 2,
        N = f * y + x;
      for (let M = p.to - 1; M >= p.from; --M) {
        ...
        if (z) {
          ...
          const W = Math.round(z.nt * g) + x,
            A = z.st * y;
          S.moveTo(W, A), S.arc(W, A, N, 0, 2 * Math.PI)
        }
      }
      S.fill()
    }(t, e, l, i, c)
  ...
  K({
    ...
    for (let r = this.zt.tt.from; r < this.zt.tt.to; ++r) {
      ...
      if (o) {
        ...
        const V = a + S;
        let x = Math.max(v, Math.round(h.pe * i) - l),
          N = x + u - 1;
        N > v + y - 1 && (N = v + y - 1, x = N - u + 1), t.fillRect(d + 1, x, V - d, N - x + 1)
      }
    }
  }
  oe(t) {
...
class $h {
  ...
  me(t, e, i) {
    ...
    for (let p = u; p < d; p++) {
      ...
      let x = s + c * (g - r),
        N = a ? x : this.ro - 1 - x;
      v.ve = N, x = s + c * (y - r), N = a ? x : this.ro - 1 - x, v.ce = N, x = s + c * (S - r), N = a ? x : this.ro - 1 - x, v.de = N, x = s + c * (V - r), N = a ? x : this.ro - 1 - x, v.pe = N
    }
  }
  pn(t, e) {
    const i = this.Co(t, e);
      ...
      const h = s.$o(),
        l = function(a, c, u) {
          ...
          for (const S of a) {
            ...
            for (const x of V) g = x.zOrder, (!(y = p == null ? void 0 : p.zOrder) || g === "top" && y !== "top" || g === "normal" && y === "bottom") && (p = x, v = S)
          }
          var g, y;
          return p && v ? {
            Iv: p,
  ...
  Nl = Z({
    ...
    setup(n) {
      const t = n,
        ...
        u = (d, f) => {
          var x, N;
          const p = (x = Bs().$webApp.dataUnsafe.user) == null ? void 0 : x.language_code,
            v = (N = i.value) == null ? void 0 : N.timeScale().getVisibleLogicalRange();
    ...
    setup(n) {
      ...
      const ut = async () => {
        ...
        const P = C.address,
          ...
          B = {
            jettonBalance: x.value ? fi(x.value, C.ticker) : void 0,
      ...
      }), St(() => k.value.status, (C, P) => {
        C === "moving_to_dex" ? d.internal.forceHideTabs() : P === "moving_to_dex" && d.internal.removeForceHideTabs()
      });
        ...
        const C = D(),
          ...
          U = Yt(async () => {
            ...
            return ft({
              ...
              transactions: (Pt(K) ? K.data : P.value) || P.value
            })
          }, J => {
            C.value = J.information, P.value = J.transactions
          }, 5e3);
        return {
          init: async () => await U.exec(),
          destroy: () => {
            U.destroy(), C.value = void 0, P.value = void 0
          },
          update: async () => await U.exec(),
          jettonInformation: T(() => C.value),
          jettonTransactions: T(() => P.value)
        }
      }
      ...
      function Q(C) {
        ...
        const O = () => {
          ...
          rt = Yt(async () => {
            ...
            return Y.signal.aborted ? ft(P.value) : ft((Pt(Mt) ? Mt.data : P.value) || P.value)
          }, Mt => P.value = Mt, 5e3);
        return {
          init: async () => await rt.exec(),
          destroy: () => {
            rt.destroy(), P.value = void 0, U.value = Qt.ONE_MINUTE
          },
      ...
      function jt() {
        ...
        function Ii() {
          Y = St(k, at => {
            ...
            at.status === "error" || at.status === "loading" || O || !at.jettonTrading || l.value !== "connected" || (J == null || J(), O = Yt(async () => {
              ...
              return ft((Pt(Xt) ? Xt.data : P.value) || P.value)
            }, dt => P.value = dt, 6e4), O == null || O.exec())
          }, {
            immediate: !0
          }), rt = St(k, at => {
        ...
        return {
          init: Ii,
          destroy: () => {
            Y == null || Y(), B == null || B.destroy(), J == null || J(), O == null || O.destroy(), rt == null || rt(), K == null || K.destroy(), C.value = void 0, P.value = void 0, U.value = void 0
          },
          update: async () => {
            await (B == null ? void 0 : B.exec()), await (O == null ? void 0 : O.exec()), await (K == null ? void 0 : K.exec())
          },
          resetAndUpdateUnlocked: async () => {
            U.value = void 0, await (K == null ? void 0 : K.exec())
          },
          jettonBclData: T(() => C.value),
          jettonBalance: T(() => P.value),
          jettonUnlockEnabled: T(() => U.value)
        }
      }

      function zt() {
        const C = [et.TRADE, et.SLIPPAGE, et.IMAGE_PREVIEW],
          P = D(void 0),
          U = {
            update: B => {
              P.value = B
      ...
      return (C, P) => {
        ...
        return _(), L("div", oc, [m(k).status === "error" ? (_(), R(U, {
          ...
          onButton: P[0] || (P[0] = rt => ("visitCommunity" in C ? C.visitCommunity : m(nc))())
          ...
          "is-released": ""
        }, null, 8, ["jetton", "jetton-information", "unlock-enabled", "balances", "jetton-management"])) : (_(), R(O, {
          key: 1,
          jetton: m(k).jetton,
          "jetton-information": m(k).jettonInformation,
