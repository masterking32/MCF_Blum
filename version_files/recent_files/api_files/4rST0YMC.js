    ...
    setup(e, {
      ...
      const t = e,
        ...
        i = R(() => {
          ...
          throw he("Unsupported condition")
        });
      return (o, p) => {
        const v = ee,
          f = ze,
    ...
    setup(e, {
      ...
      const o = R(() => {
        switch (c.value) {
          ...
          default:
            throw c.value, he("Invalid status")
        }
      });
      return (p, v) => {
        const f = ee,
...
async function pa(e, l) {
  for (const t of l) switch (t.type) {
    ...
    case "bg-image":
      ...
      break;
    default:
      throw he(`Unknown element type: ${t}`)
  }
}
    ...
    setup(e, {
      ...
      const t = l,
        ...
        v = async () => {
          ...
          h && (o.value.appendChild(h.canvas), c.value = {
            type: "loaded",
            story: h
          })
        }, f = async () => {
      ...
      return oe(() => [s.currentStreakDays.value, o.value, a.value], d, {
        ...
        const k = wt,
          $ = ge,
          w = Pe;
        return u(), S("div", ha, [n(c).type === "loading" ? (u(), I(k, {
          key: 0,
          fixed: ""
        })) : (u(), I(w, {
          key: 1,
    ...
    setup(e, {
      ...
      const t = e,
        ...
        v = () => {
          ...
          if (t.task.type === F.INTERNAL && t.task.subType === Oe.SHARE_STORIES && (o.value = !0), t.task.type === F.ONCHAIN_TRANSACTION) {
            const b = async () => {
              ...
              const T = De().getters.mustGetUser.value.id,
                P = `${t.task.id}:${T}`;
        ...
        r = R(() => {
          ...
          if ((t.task.kind === V.QUEST && t.task.reward.isZero() || t.task.kind === V.GROUP) && "subTasks" in t.task) {
            const T = ((b = t.task.subTasks) == null ? void 0 : b.reduce((L, Y) => L.add(Y.reward), new ve(0))) ?? new ve(0),
      ...
      return oe(g, b => Me().meta.back = b), (b, A) => {
        ...
        const T = Zt,
          P = na,
          L = Aa;
        return u(), S(K, null, [ae(b.$slots, "default", nt(it(n(y)))), E(T, {
          "model-value": !!n(m),
    ...
    setup(e) {
      ...
      return (o, p) => {
        ...
        return u(), I(r, {
          ...
          }, null, 8, ["claim-override", "status", "kind", "start-callback", "claim-callback", "mode", "item-mode", "onOpen:verify"])) : O("", !0), h.value ? (u(), S(K, {
            key: 1
          }, [h.value.done ? (u(), I(f, {
            key: 0,
    ...
    setup(e) {
      ...
      return de(() => v.disconnect()), (r, m) => {
        ...
        return re((u(), I(we, {
          ...
          default: N(() => [(u(!0), S(K, null, W(e.tasks, (w, h) => (u(), I(k, {
            key: w.id,
      ...
      const l = qe(),
        ...
        i = R(() => {
          ...
          return s.value.map(g => {
            const b = y(g.tasks).map(T => ({
                ...T,
                subTasks: T.subTasks ? y(T.subTasks) : T.subTasks
              })),
              A = g.subSections.map(T => ({
                ...T,
                tasks: y(T.tasks).map(P => ({
                  ...P,
        ...
        o = D({}),
        v = bt(async () => Tt(await te.getTasksSections(j().$webApp.dataUnsafe.query.ref)), h => s.value = h, 15e3),
        f = (h, y) => {
          if (i.value)
            for (const g of i.value) {
              const A = [...g.tasks, ...g.tasks.flatMap(T => T.subTasks ?? []), ...g.subSections.flatMap(T => T.tasks ?? [])].filter(T => T.id === h);
              if (A.length) {
                A.forEach(T => {
                  Object.assign(T, y)
                });
        ...
        d = async h => {
          const y = await te.startTask(h.id);
          y.err || f(h.id, y.data)
        }, r = async h => {
          const y = await te.claimTask(h.id);
          return y.err ? !1 : (f(h.id, y.data), gt().update(), !0)
        }, m = async (h, y) => {
          const g = await te.verifyTask(h.id, y);
          return g.err ? g.code === 400 ? z.WRONG_KEYWORD : z.UNKNOWN_ERROR : (f(h.id, g.data), z.SUCCESS)
        }, k = h => {
          setTimeout(() => delete o.value[h.id], 3e3), h.reward.isZero() || ht().success(_("base.you_got_bp", {
            bp: se(h.reward)
          }))
        }, $ = R(() => i.value ? i.value.reduce((y, g) => ([...g.tasks, ...g.tasks.flatMap(T => T.subTasks ?? []), ...g.subSections.flatMap(T => T.tasks ?? [])].filter(T => !X(T.status, o.value[T.id]) && !Ue(T) && T.kind !== V.GROUP).map(T => T.id).forEach(T => y.add(T)), y), new Set).size : 0);
      return (async () => {
        ...
        const h = i.value.flatMap(y => y.tasks).find(y => y.id === t.value && [V.QUEST, V.GROUP].includes(y.kind));
        h && yt(() => Fe().openTaskSheet(h))
      }), de(() => {
        v.destroy()
      }), (h, y) => {
        const g = Bt,
        ...
        return u(), S("div", ps, [n(a) ? (u(), I(g, {
          ...
          key: 1
        }, [C("div", fs, [C("span", vs, [be(U(("t" in h ? h.t : n(_))("earn.title")) + " ", 1), C("sup", null, U(n($)), 1)])]), C("div", bs, [(u(!0), S(K, null, W(n(i), (T, P) => (u(), I(b, {
          key: P,
